...
async function getObjectsArrayByIds(idArray,fields,limit,fetchObjectArray=null){
   if(idArray.length === 0){
    return [];
   }
   else{ 
    ...
    const response = await fetchObjectArray(fields, limit, idArray);
 ...
 async function createActivity(userId, postId, fetchData, addData, action){
    ...
    if(actionTypes.includes(action)){
        ...
        const activityObject= {
            ...
            userNiceName: user.userfullName,
            privacy: post.privacy,
            user_picture_xl: user.picture_xl,
            user_name: user.user_name,	
            action_type: action, 
            description: {
                post_thumbnail:post.thumbnail.cover,
                body:body
            }
        }
...
module.exports = {
   ...
   unFollow: async function (userId, otherUserId, fetchData, updateData){
    ...
    getFeed: async function(userId,fields,limit,fetchData,fetchObjectArray){
       limit = parseInt(limit); 
